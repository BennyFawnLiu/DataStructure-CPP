...
## 注意点
要理解“顺序表的长度是last+1”，我们可以从**下标定义和元素计数**的角度分析：

在顺序表的存储结构中，`last` 表示的是**最后一个数据元素的下标**。比如在图中，最后一个元素是 `aₙ`，它的下标是 `n-1`，这里的 `last` 就是 `n-1`。

而顺序表的长度是**数据元素的个数**。从下标 `0` 到下标 `last`（即 `n-1`），一共有多少个元素呢？我们可以举例计算：
- 当下标 `last=0` 时，元素只有 `a₁`，个数是 `1`，即 `0+1=1`；
- 当下标 `last=1` 时，元素有 `a₁、a₂`，个数是 `2`，即 `1+1=2`；
- 当下标 `last=n-1` 时，元素有 `a₁、a₂、…、aₙ`，个数是 `n`，即 `(n-1)+1 = n`。

所以，**数据元素的个数 = 最后一个元素的下标（last） + 1**，也就是“顺序表的长度 = last + 1”。
